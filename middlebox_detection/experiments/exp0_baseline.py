"""
EXP 0 — Baseline (no middlebox)
Purpose: Verify clean path has Dl < 5%
Expected: Median Dl < 5%, NO MIDDLEBOX
Run: sudo python3 experiments/exp0_baseline.py
"""
...
def run():
    ...
    try:
        # Save tc config (baseline — no MB enabled)
        with open(os.path.join(RESULTS, 'tc_config.txt'), 'w') as f:
            f.write('--- BEFORE (no MB) ---\n')
        ...
        recv_proc.stdout.readline()   # wait for READY
        time.sleep(0.5)
