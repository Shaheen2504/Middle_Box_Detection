"""
EXP 7 — Shaper + Compressor + SPQ (three-chain)
Purpose: Detect all 3 chained MBs
Expected: Median Dl complex, MIDDLEBOX DETECTED
Run: sudo python3 experiments/exp7_three_chain.py
"""
import os, sys, time, subprocess, json
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from mininet.log import setLogLevel
from topology.three_chain_topo import create_three_chain_topo
...
def run():
    ...
    net, h1, h2, rn1, rn2, r_mb1, rn3, r_mb2, rn4, r_mb3, rn5 = create_three_chain_topo()
    iface1 = 'r_mb1-eth1'
    iface2 = 'r_mb2-eth1'
    iface3 = 'r_mb3-eth1'
    daemon_proc = None
    ...
    try:
        with open(os.path.join(RESULTS, 'tc_config.txt'), 'w') as f:
            f.write('--- BEFORE ---\n')
            for node, iface in [(r_mb1, iface1), (r_mb2, iface2), (r_mb3, iface3)]:
                f.write(f'{node.name}:\n')
                f.write(node.cmd(f'tc qdisc show dev {iface}'))

        enable_shaper(r_mb1, iface1)
        daemon_proc = enable_compressor(r_mb2, iface2, sigma_mbps=2,
                                         dst_port=DST_PORT, queue_num=1)
        enable_spq(r_mb3, iface3)
        ...
        with open(os.path.join(RESULTS, 'tc_config.txt'), 'a') as f:
            ...
            for node, iface in [(r_mb1, iface1), (r_mb2, iface2), (r_mb3, iface3)]:
                f.write(f'{node.name}:\n')
                f.write(node.cmd(f'tc qdisc show dev {iface}'))
                f.write(node.cmd(f'tc filter show dev {iface}'))

        with open(os.path.join(RESULTS, 'iptables_config.txt'), 'w') as f:
            f.write(r_mb2.cmd('iptables -L FORWARD -v -n'))
        ...
        disable_shaper(r_mb1, iface1)
        disable_compressor(r_mb2, iface2, daemon_proc)
        disable_spq(r_mb3, iface3)
        net.stop()
